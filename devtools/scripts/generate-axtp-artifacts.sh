...
const { emitDartFiles } = await import(pathToFileURL(path.join(root, "devtools/generators/dist/emitters/dart.js")).href);

const spec = await loadProtocolSources(specRoot);
for (const message of validateSpec(spec)) console.log(message);

const outDir = path.join(root, "lib/src/generated");
await emitDartFiles(spec, outDir);
console.log(`[OK] generated Dart artifacts: ${outDir}`);
NODE

AXTP_SPEC_PATH="$spec" node "$root/devtools/scripts/axtp-versioning.mjs" generate --runtime-name axtp-flutter-runtime
